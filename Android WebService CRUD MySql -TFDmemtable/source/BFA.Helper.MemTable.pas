...
type
  TFDMemTableHelper = class helper for TFDMemTable
    ...
    function  FillDataFromURL(FURL : String; FMultiPart : TMultipartFormData = nil) : Boolean;
  end;
...
begin
  try
    ...
    if TJSONObject.ParseJSONValue(FJSON) is TJSONObject then begin
      ...
      Result := False;
      Self.FillError('Parsing JSON Failed', 'Invalid JSON');
      Exit;
    end;
...
begin
  ...
  try
    try
      ...
      on E : Exception do begin
        Result := False;
        Self.FillError('Failed Loading JSON From URL', E.Message);
      end;
    end;
  finally
    FNetHTTP.DisposeOf;
